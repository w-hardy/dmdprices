...
`dmdprices` provides price lookups against the NHS Dictionary of Medicines and Devices (dm+d). A bundled dataset (Week 34 2025) is included so no setup is needed for immediate use.
...
| `dmd_dose_optimise()` | Find the cheapest / minimum-item combination that delivers a clinical dose |
| `dmd_parse_strength()` | Parse the strength out of a VMP name |
...
Returns all rows where the medicine name contains the query string (case-insensitive):
...
By default, rows where both `basic_price` and `nhs_indicative_price` are `NA`
